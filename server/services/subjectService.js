...
function isRevisionDue(topic, progress, now) {
  ...
  const intervalDays =
    topic.revisionFrequencyDays || DEFAULT_REVISION_INTERVAL_DAYS;
  const dueAt =
    new Date(progress.lastRevised).getTime() +
...
export const subjectService = {
  ...
  async listWithProgress(userId) {
    ...
    const progressDocs = await UserTopicProgress.find({
      user: userId,
      subject: { $in: subjectIds },
    });

    const progressByTopic = new Map(
      progressDocs.map((p) => [String(p.topic), p])
    );
    ...
    return subjects.map((s) => {
      ...
      for (const topic of subjectTopics) {
        ...
        if (
          ...
          (!lastRevised || progress.lastRevised > lastRevised)
        ) {
      ...
      return {
        ...
        revisionDueTopics: {
          count: revisionDueTopics.length,
          topics: revisionDueTopics,
        },
      };
    });
  },
  ...
  // merged with this user's progress and its learning resources/practice.
  async getSubjectDetail(userId, subjectId) {
    ...
    const [progressDocs, resources, practice] = await Promise.all([
    ...
    for (const resource of resources) {
      const key = String(resource.topic);

      if (!resourcesByTopic.has(key)) {
        resourcesByTopic.set(key, []);
      }

      resourcesByTopic.get(key).push(resource);
    }

    const practiceByTopic = new Map();

    for (const practiceSet of practice) {
      const key = String(practiceSet.topic);

      if (!practiceByTopic.has(key)) {
        practiceByTopic.set(key, []);
      }

      practiceByTopic.get(key).push(practiceSet);
    }

    const topicsWithProgress = topics.map((topic) => {
      const topicKey = String(topic._id);
      const progress = progressByTopic.get(topicKey);
      ...
      return {
        ...
        lastRevised: progress?.lastRevised || null,

        resources: resourcesByTopic.get(topicKey) || [],
        practice: practiceByTopic.get(topicKey) || [],
      };
    });
  ...
  // "touching" the topic and bumps lastRevised, same convention as
  // the DSA Tracker's revision logic.
  ...
  // numbers are computed exactly once and stay consistent between
  // the cards and this summary.
  async getOverallAnalytics(userId) {
    const subjects = await this.listWithProgress(userId);

    const totalTopics = subjects.reduce(
      (sum, s) => sum + s.totalTopics,
      0
    );
    ...
    const totalRemainingTopics =
      totalTopics - totalCompletedTopics;

    const totalRevisionDueTopics = subjects.reduce(
      (sum, s) => sum + s.revisionDueTopics.count,
      0
    );

    const overallReadiness = totalTopics
    ...
    const strongestSubject = ranked.length
      ? ranked.reduce((a, b) =>
          b.progressPercent > a.progressPercent ? b : a
        )
      : null;
    ...
    return {
      ...
      strongestSubject: strongestSubject
        ? toSubjectSummary(strongestSubject)
        : null,

      weakestSubject: weakestSubject
        ? toSubjectSummary(weakestSubject)
        : null,
      ...
      suggestedNextSubject: weakestSubject
        ? toSubjectSummary(weakestSubject)
        : null,
    };
  },
  ...
  // studies but doesn't toggle anything that day won't register.
  // A precise, cross-feature streak would need its own StudyActivity/
