...
export function CompanyCard({ company }) {
  return (
    <Link
      to={ROUTES.COMPANY_DETAILS.replace(
        ":companyId",
        company._id
      )}
      className="block"
      ...
      <Card className="p-5 transition-colors hover:bg-white/[0.02]">
        <div className="flex items-start gap-3">
          {company.logoUrl ? (
            ...
            <div className="flex size-10 shrink-0 items-center justify-center rounded-xl bg-primary-muted font-display text-[13px] font-semibold text-primary-strong">
        ...
        {(company.importantTopics?.length > 0 ||
          ...
          <div className="mt-3 flex items-center gap-4 border-t border-border pt-3 text-[11.5px] text-text-faint">
            {company.importantTopics?.length >
              0 && (
              <span>
                {company.importantTopics.length} key topics
              </span>
            )}

            {company.interviewRounds?.length > 0 && (
              <span>
                {company.interviewRounds.length} interview rounds
              </span>
            )}
          </div>
        )}
